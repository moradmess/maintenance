 ...
 if(isset($_GET['word']))
            {
              ...
              header("Content-Disposition: attachment; filename=Historique_MP_Access2.doc");  
            }
            else
            {
              header("Content-Type: application/xls");
              header("Content-Disposition: attachment; filename=Historique_MP_Access2.xls");   
            }
...
<?php
    echo '<table>';
       ...
       $req = $bdd->query('SELECT dm.serial_number,dm.version_soft,dm.apf,dm.date_installe,c.nom_client,c.ville,dm.contrat,dm.date_mp, dm.nom_intervenant, dm.detail FROM histo_access2 dm INNER JOIN client c ON dm.code_client=c.idc');
       $donnee = $req->fetchAll();
